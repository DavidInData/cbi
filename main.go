...
//https://data.cityofchicago.org/resource/xhc6-88s9.json
...
func init() {
	...
	//db_connection := "user=postgres dbname=chicago_business_intelligence password=S0mething host=localhost sslmode=disable port = 5432"
...
func GetTaxiTrips(db *sql.DB) {
	...
	geocoder.ApiKey = "AIzaSyAMhJjAg-OnKLfWdTslqSY39MgL0JwJhcQ"
...
func GetCovidDetails(db *sql.DB) {
	//https://data.cityofchicago.org/resource/yhhz-zm2v.json?$limit=5
	fmt.Println("GetCovidDetails: Collecting GetCovidDetails")
...
func GetCCVIDetails(db *sql.DB) {
	//https://data.cityofchicago.org/resource/xhc6-88s9.json?$limit=500
	fmt.Println("GetCCVIDetails: Collecting GetCCVIDetails")
	...
	for i := 0; i < len(ccvi_data_list); i++ {
		...
		sql := `INSERT INTO ccvi_details ("community_area_or_zip", 
			"geography_type",  
			"community_area_name",  
			"ccvi_score",  
			"ccvi_category" )
